...
#define delay         10000
std::string yml_path = "C:/QuixMind/QuixSmart/configuraciones/robotmodel/";
...
class RobotModelNode : public rclcpp::Node{
  public:
      ...
      RobotModelNode(): Node("RobotModelNode"){
          ...
          posVehSub = this->create_subscription<example_interfaces::msg::Float64MultiArray>("/posVeh", 0, bind(&RobotModelNode::posVeh_subscriber_callback_simplemodel, this, _1));
          ...
          //se inicializa la primera posicion
          // x[0] = xp[0];
          // y[0] = xp[1];
          step_now++;
  ...
  private:
    /**
     * @brief Subscriber callback. Will subscribe to a topic called /posVeh
    ...
    void posVeh_subscriber_callback(const example_interfaces::msg::Float64MultiArray::SharedPtr msg){
      ...
      if(first_iteration){
        ...
        a = actual_position->data[0]; //pos X
        b = actual_position->data[1]; //pos Y
        theta_0 = actual_position->data[2] * 180/PI; //Theta en grados
        ...
        //std::cout << "x: " << xp[0] << " ,y: " << xp[1] << " ,theta: "<< xp[2] << std::endl;

        t = std::chrono::steady_clock::now();
        
        double t_milliseconds = std::chrono::duration_cast<std::chrono::milliseconds>(t - t0).count();
        // Posicion transformada con origen en la posicion inicial del movimiento
        float tf_pose[3];
        tf_position(xp[0], xp[1], xp[2], a, b, theta_0, tf_pose);
        aux.pose[0] = tf_pose[0];
        aux.pose[1] = tf_pose[1];
        aux.pose[2] = tf_pose[2];
        // Posicion raw sin transformacion al origen
        // datos[step_now].pose[0] = xp[0];
        // datos[step_now].pose[1] = xp[1];
        // datos[step_now].pose[2] = xp[2];

        //std::cout << "x: " << tf_pose[0] << " ,y: " << tf_pose[1] << " ,theta: "<< tf_pose[2] << " // Pose despues de tf " << std::endl;
        ...
        float dx = datos.back().pose[0] - datos[datos.size()-2].pose[0]; //Incremento de x con respecto a la posicion anterior
        float dy = datos.back().pose[1] - datos[datos.size()-2].pose[1]; //Incremento de y con respecto a la posicion anterior
        ...
        // Evita variaciones minimas y maximas de posicion debido al error de medicion
        if(dd>0.001){
          if(dd>0.2){ // Si el incremento de posicion es mayor a un umbral se considera error de medicion y se elimina toda esa medicion del vector de datos
            std::cout<<"pop back"<<std::endl;
            datos.pop_back();
            step_now = step_now - 1;
          }else{ // Si dd es menor al umbral se suma a la distancia acumulada y se actualiza la distancia en el vector de datos
            d += dd;
            datos[step_now].d = d;
          }
        }else{
        ...
        if(d > d_max){
          if (u[0] >= 0) { // Superada con velocidad positiva
            d = 0;
            dd = 0;
            a = xp[0];
            b = xp[1];
        ...
        myfilex.open("vector_x.txt",std::ios::trunc);
        myfiley.open("vector_y.txt",std::ios::trunc);
        myfiletheta.open("vector_theta.txt",std::ios::trunc);
        myfile_t.open("vector_t.txt",std::ios::trunc);
        myfile_d.open("vector_d.txt",std::ios::trunc);
        ...
        //Copiar a un archivo de texto las posiciones x y
        /*float matrix[2];
        matrix[0] = new float[step_now];
        matrix[1] = new float[step_now];
        */
        ...
        //Get yml file
        Yml yml_model("configuraciones/model.yml");
        //Load yml file
    ...
    /**
     * @brief Subscriber callback pero solo hace la parte del modelo con y+. Will subscribe to a topic called /posVeh
     * @param  msg Mensaje publicado en el topic /posVeh que contiene un array de 3 floats, [0] x(m), [1] y(m), [2] theta(rad)
     * @return  none
     */
    void posVeh_subscriber_callback_only_forward(const example_interfaces::msg::Float64MultiArray::SharedPtr msg){
      if((ciclo_last != ciclo) || (option_last != option)){
        example_interfaces::msg::Float64MultiArray to_publish;
        u[0] = 0;
        to_publish.data = {u[0],u[1]};
    ...
    /**
     * @brief Subscriber callback pero solo hace el modelo con y-. Will subscribe to a topic called /posVeh
     * @param  msg Mensaje publicado en el topic /posVeh que contiene un array de 3 floats, [0] x(m), [1] y(m), [2] theta(rad)
     * @return  none
     */
    void posVeh_subscriber_callback_only_backward(const example_interfaces::msg::Float64MultiArray::SharedPtr msg){
      if((ciclo_last != ciclo) || (option_last != option)){
        example_interfaces::msg::Float64MultiArray to_publish;
        u[0] = 0;
        to_publish.data = {u[0],u[1]};
      ...
      if(first_iteration){
        ...
        if(d > d_max){
          ...
          write_yml_groups(yml_parameters, arco, u[0], u[1]);
          u[0] = -v*3;
          u[1] = giro_max - delta_giro*ciclo; // Los motores marcha atras son mas lentos lo que provocan que el angulo de giro hacia adelante y hacia atras no sea el mismo
          std::cout<<"\n\n\n\n---------cambio de sentido a la opcion = 1, marcha atras--------\n\n\n\n\n"<<std::endl;
          arco++;
          step_now++;
          aux.pose[0] = 0;
        ...
        std::ofstream myfile_t;
        std::ofstream myfile_d;
        myfilex.open("vector_x_backward.txt",std::ios::trunc);
        myfiley.open("vector_y_backward.txt",std::ios::trunc);
        myfiletheta.open("vector_theta.txt",std::ios::trunc);
        myfile_t.open("vector_t.txt",std::ios::trunc);
        myfile_d.open("vector_d.txt",std::ios::trunc);
        if(myfilex.is_open() && myfiley.is_open()){
     ...
     * @return  none
     */
    void posVeh_subscriber_callback_simplemodel(const example_interfaces::msg::Float64MultiArray::SharedPtr msg){
    ...
    void simple_callback(const example_interfaces::msg::Float64MultiArray::SharedPtr msg){
      std::cout << "simple callback: {" << msg->data[0] << ", " << msg->data[1] << ", " << msg->data[2] << "}" << std::endl;
      // std::cout<<step_now<<" step_now";
      example_interfaces::msg::Float64MultiArray control;
      static float traction = 40;
      static float steering = -45;
      control.data = {traction,steering};
      RobotModelControlPub->publish(control);
...
// /////////////////////////////////////////////
// //MODELO DE VEHICULO EMPLEADO PARA SIMULAR EL 
// //MOVIMIENTO DE ESTE EN TELECONTROL SSIMULADO
// /////////////////////////////////////////////

// static void modelT2(float*, float*, float);


// static void modelT2(float* x, float* u, float dt)
// {
// 	double xin, yin, orin;
// 	double xout, yout, orout;
// 	double xsig, ysig, orsig;
// 	double distEjes = 1.0;
...
// 	xsig = xin + (dt * u[1] * cos(orin) * cos(u[0]));
// 	ysig = yin + (dt * u[1] * sin(orin) * cos(u[0]));
// 	orsig = orin + (dt * u[1] * sin(u[0]) / distEjes);
